...
export class ViteJasminePreprocessor extends EventEmitter {
    ...
    async runHeadlessBrowserTests(browserType) {
        ...
        try {
            ...
            await page.waitForFunction(() => window.jasmineFinished === true, { timeout: this.config.jasmineConfig?.env?.timeout ?? 120000 });
...
`;
    ...
    async runHeadlessTests() {
        return new Promise((resolve) => {
            ...
            child.on('close', (code) => {
                console.log(`📊 Node.js test runner exited with code: ${code}`);
    ...
    async waitForServerReady(url, timeout = 5000) {
