...
class Server {
  ...
  userJs() {
    const esmExt = this.options.esmFilenameExtension;
    ...
    const specUrls = this.getUrls(
      ...
      '/__spec__'
    ).map(url => url.replace(/\.ts$/, '.js')); // <- rewrite .ts → .js
  ...
  start() {
    ...
    if (this.options.transpileTs && this.options.server?.useMiddlewares) {
      for (const mwPath of this.options.server.useMiddlewares) {
        const mw = require(path.resolve(this.projectBaseDir, mwPath));
        app.use('/__spec__', mw); // middleware will map .js → .ts
        app.use('/__src__', mw);
      }
    }
